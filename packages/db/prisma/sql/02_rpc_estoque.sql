...
-- Remove versões antigas (assinaturas anteriores). O DROP da assinatura text é
-- necessário porque CREATE OR REPLACE não permite renomear parâmetros.
...
begin
  ...
  if v_categoria = 'FERRAMENTA' and p_tipo in ('SAIDA', 'AJUSTE') then
    select coalesce(sum(quantidade), 0) into v_emprestado
      from "emprestimos" where "produtoId" = p_produto_id and status = 'ABERTO';
  end if;
  ...
  update "produtos" set "estoqueAtual" = v_saldo where id = p_produto_id;

  insert into "movimentacoes_estoque"
    (id, "produtoId", tipo, quantidade, "saldoApos",
     "usuarioId", "ordemServicoId", motivo, "criadoEm")
  values
    (v_mov_id, p_produto_id, p_tipo::"TipoMovimentacao", p_qtd, v_saldo,
...
revoke execute on function registrar_movimentacao_estoque(text, int, text, text, text, text) from public, anon, authenticated;
grant execute on function registrar_movimentacao_estoque(text, int, text, text, text, text) to service_role;
...
begin
  ...
  select "prazoEmprestimoHoras" into v_prazo_horas from "configuracoes" where id = 'default';
  v_prazo_horas := coalesce(p_prazo_horas, v_prazo_horas, 24);

  insert into "emprestimos"
    (id, "produtoId", "funcionarioId", quantidade, status, "retiradoEm", "prazoEm", "usuarioRetiradaId", motivo)
  values
    (v_emp_id, p_produto_id, p_funcionario_id, p_qtd, 'ABERTO', now(),
     now() + (v_prazo_horas || ' hours')::interval, p_usuario_id, p_motivo);

  insert into "movimentacoes_estoque"
    (id, "produtoId", tipo, quantidade, "saldoApos", "usuarioId", "emprestimoId", motivo, "criadoEm")
  values
    (v_mov_id, p_produto_id, 'EMPRESTIMO'::"TipoMovimentacao", p_qtd, v_estoque, p_usuario_id, v_emp_id, p_motivo, now());
...
begin
  select "produtoId", quantidade into v_produto_id, v_qtd
    from "emprestimos" where id = p_emprestimo_id and status = 'ABERTO'
    for update;
  if not found then
  ...
  select "estoqueAtual" into v_saldo from "produtos" where id = v_produto_id;

  insert into "movimentacoes_estoque"
    (id, "produtoId", tipo, quantidade, "saldoApos", "usuarioId", "emprestimoId", motivo, "criadoEm")
  values
    (v_mov_id, v_produto_id, 'DEVOLUCAO'::"TipoMovimentacao", v_qtd, v_saldo, p_usuario_id, p_emprestimo_id, p_motivo, now());
...
begin
  ...
  update "produtos" set "estoqueAtual" = "estoqueAtual" - v_qtd
   where id = v_produto_id and "estoqueAtual" >= v_qtd
  ...
  insert into "movimentacoes_estoque"
    (id, "produtoId", tipo, quantidade, "saldoApos", "usuarioId", "emprestimoId", motivo, "criadoEm")
  values
    (v_mov_id, v_produto_id, 'SAIDA'::"TipoMovimentacao", v_qtd, v_saldo, p_usuario_id, p_emprestimo_id, 'Perda: ' || p_motivo, now());
...
revoke execute on function registrar_emprestimo(text, int, text, text, int, text) from public, anon, authenticated;
revoke execute on function registrar_devolucao(text, text, text) from public, anon, authenticated;
revoke execute on function registrar_perda(text, text, text) from public, anon, authenticated;
grant execute on function registrar_emprestimo(text, int, text, text, int, text) to service_role;
grant execute on function registrar_devolucao(text, text, text) to service_role;
grant execute on function registrar_perda(text, text, text) to service_role;
